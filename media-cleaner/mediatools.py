    ...
    for movie_name in os.listdir(root_dir):
        ...
        if os.path.isfile(os.path.join(root_dir, movie_name)):
            ...
            op_counter = _merge_op_counts(op_counter, \
                _move_file_dir(flags, os.path.join(root_dir, movie_name), \
        ...
        for dir_path, _, files in os.walk(current_dir):
            for file_ in files:
                ...
                if _is_main_file(file_, dir_path):
                    ...
                    op_counter = _merge_op_counts(op_counter, \
                        _clean_movie_main_file(flags, dir_path, file_, \
        ...
        for dir_path, _, files in os.walk(current_dir):
            for file_ in files:
                if _has_markers(file_) and \
                    ...
                    # Clean tv main file name.
                    op_counter = _merge_op_counts(op_counter, \
                        _clean_tv_main_file(flags, current_dir, dir_path, \
                                            file_, series_name))
                else:
                    op_counter = _merge_op_counts(op_counter, \
                        _clean_other_file(flags, current_dir, dir_path, file_))
...
def _clean_other_file(flags, base_dir, dir_path, file_):
    ...
    if _is_extras_file(file_, dir_path):
        ...
        op_counter = _merge_op_counts(op_counter, \
            _move_file_dir(flags, os.path.join(dir_path, file_), \
...
def _is_sample_file(file_, path):
    ...
    return os.path.getsize(os.path.join(path, file_)) < _MIN_VIDEO_SIZE or \
           (match != None and \
           os.path.getsize(os.path.join(path, file_)) < _MIN_MAIN_VIDEO_SIZE)
...
def _is_extras_file(file_, path):
    ...
    return (_is_video_file(file_) and not _is_sample_file(file_, path) and \
            ((os.path.getsize(os.path.join(path, file_)) < \
                ...
                not _has_markers(file_)) or match != None)) or \
...
def _clean_tv_main_file(flags, series_dir, dir_path, file_, series_name):
    ...
    op_counter = _move_file_dir(flags, os.path.join(dir_path, file_), \
...
def _clean_tv_main_file_name(flags, dir_path, file_, series_name):
    ...
    return _move_file_dir(flags, os.path.join(dir_path, file_), \
...
def _get_clean_movie_dir_name(movie_name, dir_):
    ...
    match = _get_movie_name_year_match(movie_name)

    # If the name might be incorrect, check for possible alts.
    if (match == None or not _is_valid_media_name(match[0])) \
            and os.path.isdir(dir_):
        match = _find_movie_name_year_match(dir_)
...
def _find_movie_name_year_match(dir_):
    ...
    for name in names_to_check:
        match = _get_movie_name_year_match(name)
        if match != None and _is_valid_media_name(match[0]):
            return match
    return None
...
def _remove_empty_folders(flags, path, remove_root=True):
    ...
    if len(files):
        for file_ in files:
            ...
            if os.path.isdir(fullpath):
                op_counter = _merge_op_counts(op_counter, \
                    _remove_empty_folders(flags, fullpath))
